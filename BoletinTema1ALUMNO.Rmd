...
#Hay 11 animales distintos, 8 de ellos se encuentran en mammels y en Animals2 pero esto es debido a que están escritos de forma diferente en ambos conjuntos y los otros 3, que se encuentran en Animals2 son dinosaurios.
...
#Ambos conjuntos presentan comportamientos similares y podemos afirmar, observando la gráfica, que sí siguen la distribución gaussiana.
```
...
3.  La función **library** sin argumentos abre una ventana y muestra las librerías que han sido instaladas.
    ...
    length(libReturn$results)/3
    #También se podría usar head(libReturns), aunque no daría directamente el nº de librerías.
...
#is.na(cabbages)
#Mediante el uso de any obtendremos solo un booleano.
any(is.na(C))
...
#Como se observa, ambas variables del conjunto siguen la distribución gaussiana.
...
```{r}
#Podemos crear el histograma mediante la función ggplot(al haber dos variables será más fácil mediante este comando).

library(ggplot2)

ggplot(data.frame(cabbages), aes(VitC, fill=Cult)) + 
       geom_histogram(color = "black")

#Como se observa en el histograma, la variedad c39 presenta, en general, menos cantidad de Vitamina C que la variedad c52 de calabazas.
```
...
#Diagrama de dispersión según la clase de calabaza:
plot(cabbages$HeadWt, cabbages$VitC, pch=c(0,16)[cabbages$Cult])
legend(x="topright",legend=c("Cult = c39","Cult = c52"), pch=c(0,16))

#Dos modelos lineales según la clase vendrán dados por:
Mod1 <- lm(VitC ~ HeadWt, data = cabbages, subset = which(Cult == "c39"))
Mod2 <- lm(VitC ~ HeadWt, data = cabbages, subset = which(Cult == "c52"))

#Finalmente, represento las rectas de cada modelo lineal:
abline(Mod1, lty = 3)
abline(Mod2)

```

c.  Usa **summary** con cada uno de los modelos obtenidos y observa **Coefficients**. Dado que hemos planteado un modelo $y=mx+n$, donde $y=VitC$ y $x=HeadWt$. La función **lm** nos permite obtener **(Intercept)**; **n** y la pendiente **HeadWt**; **m** (además de otros parámetros adicionales que evalúan la caracterísiticas del modelo). Observa que en todos los casos, la pendiente es negativa indicando que las calabazas de más peso contienen menos vitamina C. No te preocupes por el resto de parámetros del modelo, por el momento.
